					...
					<div class="input-group">
				        <input class="form-control pull-right from"
				        		...
				        		type="number"
									...
									<?php endif; ?>
				        		>
				        <div class="input-group-append range-filter-<?php echo e($filter->key); ?>-clear-button">
...
<?php $__env->startPush('crud_list_scripts'); ?>
	<script>
		jQuery(document).ready(function($) {
			$("li[filter-key=<?php echo e($filter->key); ?>] .from, li[filter-key=<?php echo e($filter->key); ?>] .to").change(function(e) {
				e.preventDefault();
				var from = $("li[filter-key=<?php echo e($filter->key); ?>] .from").val();
				var to = $("li[filter-key=<?php echo e($filter->key); ?>] .to").val();
				if (from || to) {
					var range = {
						'from': from,
						'to': to
			...
			$('li[filter-key=<?php echo e($filter->key); ?>]').on('filter:clear', function(e) {
				...
				$("li[filter-key=<?php echo e($filter->key); ?>] .from").val("");
				$("li[filter-key=<?php echo e($filter->key); ?>] .to").val("");
				$("li[filter-key=<?php echo e($filter->key); ?>] .to").trigger('change');
			});

			// range clear button
			$(".range-filter-<?php echo e($filter->key); ?>-clear-button").click(function(e) {
				e.preventDefault();

				$('li[filter-name=<?php echo e($filter->key); ?>]').trigger('filter:clear');
			})
...
<?php /**PATH E:\Programs\OpenServer\OSPanel\domains\storeclothing\vendor\backpack\crud\src\resources\views\crud/filters/range.blade.php ENDPATH**/ ?>